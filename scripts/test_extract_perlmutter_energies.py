...
class ReferenceAuditTests(unittest.TestCase):
    ...
    def test_converged_mismatched_potentials_are_not_valid(self):
        self.parts[1]['potentials']['Ag'] = 'Ag_pv'
        status, note, raw = assess(*self.parts, metal='Ag')
        self.assertEqual(status, 'pseudopotential_mismatch')
        self.assertIn('Ag_pv', note)
        self.assertEqual(raw, -1)

    def test_gas_potential_mismatch(self):
        self.parts[2]['potentials']['C'] = 'C_h'
        self.assertEqual(assess(*self.parts, metal='Ag')[0], 'pseudopotential_mismatch')
