...
class PotentialPolicyTests(unittest.TestCase):
    ...
    def test_numeric_cells_are_validated_or_ledger_backed_and_flagged(self):
        ...
        for card in page.select('.g'):
            for tr in card.select('table tr'):
                ...
                for mode,index in [('relaxed',1),('SPE',4)]:
                    ...
                    selected=rows[key]
                    if 'historical-energy' in cells[index].get('class',[]):
                        from publication_store import read
                        if not hasattr(self,'ledger'):self.ledger={r['record_id']:r for r in read(root)}
                        record=self.ledger[cells[index]['data-record-id']]
                        self.assertEqual(value,float(record['energy_eV']))
                        self.assertIn('energy-review',cells[index]['class'])
                        self.assertTrue(selected['application_status'].startswith('historical retained:'))
                        self.assertFalse(selected['E_ads'])
                        continue
                    self.assertEqual(selected['application_status'],'selected candidate applied',key)
